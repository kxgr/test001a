text1 = {
  "product": [
    ".net_framework",
    ".net_windows_server",
    "20007_office_system",
    "27mhz_wireless_keyboard",
    "None",
    "access",
    "access_multilingual_user_interface_pack",
    "active_directory",
    "active_directory_application_mode",
    "active_directory_federation_services",
    "active_directory_lightweight_directory_service",
    "active_directory_services",
    "active_movie_control",
    "activesync",
    "activex",
    "adam",
    "all_windows",
    "ampx",
    "anti-cross_site_scripting_library",
    "antigen",
    "antigen_for_exchange",
    "antigen_for_smtp_gateway",
    "antispyware",
    "asp.net",
    "asp.net_model_view_controller",
    "asp.net_signalr",
    "atlas_framework",
    "background_intelligent_transfer_service",
    "backoffice",
    "backoffice_resource_kit",
    "baseline_security_analyzer",
    "bing",
    "biztalk_server",
    "biztalk_srv",
    "business_solutions_crm",
    "cabarc",
    "capicom",
    "chart_control_for_microsoft_.net_framework",
    "class_package_export_tool",
    "clip_art",
    "commerce_server",
    "commercial_internet_system",
    "compatibility_pack_word_excel_powerpoint",
    "compatibility_pack_word_excel_powerpoint_2007",
    "content_management_server",
    "data_access_components",
    "data_engine",
    "data_protection_manager",
    "debug_diagnostic_tool",
    "debug_interface_access_software_development_kit",
    "desktop_engine",
    "dhcp_client_service",
    "diagnostics_and_recovery_toolkit",
    "digital_image",
    "digital_image_pro",
    "digital_image_suite",
    "directx",
    "directx_files_viewer_control",
    "directx_media",
    "directx_sdk",
    "distributed_transaction_coordinator",
    "dynamics_ax",
    "dynamics_gp",
    "edge",
    "enhanced_mitigation_experience_toolkit",
    "enterprise_library",
    "entourage",
    "excel",
    "excel_2013_rt",
    "excel_for_mac",
    "excel_multilingual_user_interface_pack",
    "excel_rt",
    "excel_viewer",
    "excel_web_app",
    "exchange",
    "exchange_instant_messenger",
    "exchange_server",
    "exchange_srv",
    "expression_design",
    "expression_media",
    "expression_web",
    "fast_esp",
    "file_transfer_manager",
    "flight_simulator_x",
    "forefront_client_security",
    "forefront_endpoint_protection_2010",
    "forefront_security",
    "forefront_security_for_exchange_server",
    "forefront_security_for_sharepoint",
    "forefront_threat_management_gateway",
    "forefront_unified_access_gateway",
    "forms_server",
    "foundation_class_library",
    "frontpage",
    "frontpage_express",
    "frontpage_server_extensions",
    "ftp_service",
    "gdiplus",
    "greetings",
    "groove",
    "groove_data_bridge_server",
    "groove_management_server",
    "groove_server",
    "grpconv",
    "help_file_viewer",
    "home_publishing",
    "home_server",
    "host_integration_server",
    "hotmail",
    "html_help",
    "html_help_workshop",
    "hyperlink_object_library",
    "icm_color_management_module",
    "ie",
    "ie_for_macintosh",
    "ie_toolbar",
    "iemobile",
    "iis",
    "image_color_management",
    "index_server",
    "indexing_service",
    "infopath",
    "infotech_storage_system_library",
    "intelligent_application_gateway_2007",
    "interdev",
    "interix",
    "internet_authentication_service_helper_com_component",
    "internet_explorer",
    "internet_information_server",
    "internet_information_services",
    "internet_security_and_acceleration_server",
    "isa_server",
    "java_virtual_machine",
    "jet",
    "jscript",
    "learning_essentials",
    "live_meeting",
    "live_messenger",
    "log_sink_class_activex_control",
    "lync",
    "lync_basic",
    "lync_room_system",
    "lync_server",
    "malicious_software_removal_tool",
    "malware_protection_engine",
    "management_odata_iis_extension",
    "media_foundation_sdk",
    "media_player",
    "message_compiler",
    "message_queuing",
    "messenger_plus%21_live",
    "metadirectory_services",
    "microsoft_data_access_components",
    "microsoft_forefront_protection_2010",
    "microsoft_tech_companion",
    "mn-500_wireless_base_station",
    "modern_mail",
    "money",
    "ms-dos",
    "msde",
    "msn_chat_control",
    "msn_messenger",
    "msn_messenger_service",
    "msn_messenger_service_for_exchange",
    "msn_setup_bulletin_board_services",
    "netmeeting",
    "network_firmware",
    "network_monitor",
    "nokia_asha_501",
    "nokia_asha_501_software",
    "office",
    "office_2013_rt",
    "office_communications_server",
    "office_communicator",
    "office_compatibility_pack",
    "office_compatibility_pack_for_word_excel_ppt_2007",
    "office_converter_pack",
    "office_excel",
    "office_excel_viewer",
    "office_filter_pack",
    "office_frontpage",
    "office_groove",
    "office_groove_server",
    "office_infopath",
    "office_msodatasourcecontrol_activex",
    "office_multilingual_user_interface_pack",
    "office_onenote",
    "office_outlook",
    "office_powerpoint",
    "office_powerpoint_viewer",
    "office_project",
    "office_proofing_tools",
    "office_publisher",
    "office_sharepoint_server",
    "office_snapshot_viewer_activex",
    "office_system",
    "office_visio",
    "office_web_apps",
    "office_web_apps_server",
    "office_web_components",
    "office_word",
    "office_word_viewer",
    "office_xp",
    "onenote",
    "open_xml_file_format_converter",
    "orca",
    "organization_chart",
    "outlook",
    "outlook.com",
    "outlook_2013_rt",
    "outlook_connector",
    "outlook_express",
    "outlook_express_book_control",
    "outlook_multilingual_user_interface_pack",
    "outlook_rt",
    "outlook_web_access",
    "peachtree_accounting",
    "personal_web_server",
    "photodraw",
    "photodraw_2000",
    "picture_it",
    "pinyin_ime",
    "pinyin_new_experience_style",
    "pinyin_simple_fast_style",
    "platform_sdk",
    "plus",
    "pocket_ie",
    "pocket_internet_explorer",
    "powerpoint",
    "powerpoint_for_mac",
    "powerpoint_multilingual_user_interface_pack",
    "powerpoint_rt",
    "powerpoint_viewer",
    "powerpoint_web_app",
    "producer",
    "project",
    "project_multilingual_user_interface_pack",
    "project_portfolio_server",
    "project_professional",
    "project_server",
    "project_standard",
    "proxy_server",
    "psexec",
    "psgetsid",
    "psinfo",
    "pskill",
    "pslist",
    "psloglist",
    "pspasswd",
    "psservice",
    "psshutdown",
    "pssuspend",
    "publisher",
    "publisher_multilingual_user_interface_pack",
    "register_server",
    "remote_desktop_connection",
    "remote_desktop_connection_client",
    "remote_desktop_protocol",
    "report_viewer",
    "rich_textbox_control",
    "search_server",
    "security_compliance_manager",
    "security_essentials",
    "server",
    "server_service",
    "service_bus",
    "services",
    "sharepoint_designer",
    "sharepoint_foundation",
    "sharepoint_portal_server",
    "sharepoint_server",
    "sharepoint_server_client_components_sdk",
    "sharepoint_services",
    "sharepoint_team_services",
    "sharepoint_workspace",
    "silverlight",
    "site_server",
    "site_server_commerce",
    "skype_for_business",
    "skype_for_business_server",
    "small_business_server",
    "small_business_srv",
    "sna_server",
    "sql_server",
    "sql_server_desktop_engine",
    "sql_server_express_edition",
    "sql_server_management_studio_express",
    "sql_server_reporting_services",
    "sql_srv",
    "step-by-step_interactive_training",
    "subsystem_for_unix-based_applications",
    "sysinternals",
    "sysinternals_debugview",
    "sysinternals_pstools",
    "system_center_configuration_manager",
    "system_center_operations_manager",
    "system_information_activex_control",
    "systems_management_server",
    "systems_management_srv",
    "telnet_client",
    "teredo",
    "terminal_server",
    "tsac_activex_control",
    "urlscan",
    "vbscript",
    "vfp_ole_server_activex_control",
    "viewer",
    "virtual_machine",
    "virtual_machine_manager",
    "virtual_pc",
    "virtual_server",
    "visio",
    "visio_2000",
    "visio_2003-",
    "visio_2007",
    "visio_multilingual_user_interface_pack",
    "visio_professional",
    "visio_standard",
    "visio_viewer",
    "visual_basic",
    "visual_basic_enterprise_edition",
    "visual_basic_for_applications",
    "visual_basic_for_applications_sdk",
    "visual_basic_sdk",
    "visual_basic_software_development_kit",
    "visual_basics",
    "visual_c",
    "visual_c%23",
    "visual_c%2b%2b",
    "visual_database_tools_database_designer",
    "visual_foxpro",
    "visual_interdev",
    "visual_j%23",
    "visual_j%23_.net",
    "visual_j%2b%2b",
    "visual_studio",
    "visual_studio_.net",
    "visual_studio_team_foundation_server",
    "w3who.dll",
    "web_applications",
    "web_apps",
    "webdav_mini-redirector",
    "webtv",
    "windows",
    "windows-9x",
    "windows-ce",
    "windows-nt",
    "windows_10",
    "windows_2000",
    "windows_2000_advanced_server",
    "windows_2000_resource_kit",
    "windows_2000_terminal_services",
    "windows_2003",
    "windows_2003_server",
    "windows_2008",
    "windows_286",
    "windows_386",
    "windows_7",
    "windows_8",
    "windows_8.1",
    "windows_95",
    "windows_98",
    "windows_98_plus_pack",
    "windows_98se",
    "windows_9x",
    "windows_azure_sdk",
    "windows_ce",
    "windows_data_access_components",
    "windows_defender",
    "windows_digital_rights_management",
    "windows_essentials",
    "windows_event_viewer",
    "windows_explorer",
    "windows_help",
    "windows_image_acquisition_logger",
    "windows_installer",
    "windows_live_mail",
    "windows_live_messenger",
    "windows_live_onecare",
    "windows_mail",
    "windows_me",
    "windows_media_center",
    "windows_media_center_tv_pack",
    "windows_media_encoder",
    "windows_media_format_runtime",
    "windows_media_foundation",
    "windows_media_player",
    "windows_media_rights_manager",
    "windows_media_services",
    "windows_messaging",
    "windows_messenger",
    "windows_mobile",
    "windows_movie_maker",
    "windows_nt",
    "windows_nt_helper_components",
    "windows_nt_option_pack",
    "windows_phone",
    "windows_rt",
    "windows_rt_8.1",
    "windows_script",
    "windows_script_host",
    "windows_search",
    "windows_server",
    "windows_server_2000",
    "windows_server_2003",
    "windows_server_2008",
    "windows_server_2012",
    "windows_server_2016",
    "windows_services_for_unix",
    "windows_srv",
    "windows_terminal_services_using_rdp",
    "windows_virtual_pc",
    "windows_vista",
    "windows_xp",
    "windows_xp_media_center",
    "windowst",
    "wins",
    "winsock",
    "wmi_administrative_tools",
    "wmsde",
    "word",
    "word_automation_services",
    "word_for_mac",
    "word_multilingual_user_interface_pack",
    "word_rt",
    "word_viewer",
    "word_web_app",
    "word_web_apps",
    "wordpad",
    "wordperfect_converter",
    "works",
    "works_6-9_file_converter",
    "works_spreadsheet",
    "wyukon",
    "xbox_360",
    "xml_core_services",
    "xml_parser",
    "zero_administration_kit",
    "zune_software"
  ],
  "vendor": "microsoft"
}



def read_file(name):
    fo = open('test01.txt', 'a')
    fo.write(name)
    fo.write("\n")
    fo.close()

# print(text1.keys())
list = ()
list = text1['product']
for each in list:
    read_file(each)
    # print(text1['product'])